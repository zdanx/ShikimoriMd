...
const jadibot = async (chikimori, m, from) => {
  if (Object.keys(client).includes(from)) {
    return chikimori.sendMessage(from, {
    ...
    async function connectToWhatsApp() {
      ...
      if (usePairingCode && !client[from].user && !client[from].authState.creds.registered) {
        setTimeout(async () => {
          ...
          await chikimori.sendMessage(from, {text: txt }, {quoted: m})
        }, 2000)
      }
...
async function stopjadibot(chikimori, m, from) {
  if (!Object.keys(client).includes(from)) {
    return chikimori.sendMessage(from, {
...
async function listjadibot(chikimori, m) {
